...
<form action="{{route('products.update',$product)}}" method="POST" enctype="">
    @csrf
    @method('PUT')
    ...
    <label for="category">Category: </label>
       <select name="category_id" id="category_id">
        ...
        <option value="{{$category->id}}" {{$category->id==old('category_id',$product->category->id)?"selected":""}}>{{$category->id."-".$category->name}}</option>
